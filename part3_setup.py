...
# FONT_SMALL = pygame.font.SysFont("Comic Sans", 18)
# FONT_MED = pygame.font.SysFont("Comic Sans", 22)
...
# def draw_ui(f1:Fighter,f2:Fighter,screen):
#     pygame.draw.rect(screen, WHITE, (0, 0, sw, 120))

#     screen.blit(FONT_SMALL.render(f"{f1.name}({int(f1.health)}HP)",True,BLACK),(3 * sw // 4 -140, 6))
#     screen.blit(FONT_SMALL.render(f"{f2.name}({int(f1.health)}HP)",True,BLACK),(3 * sw // 4 -140, 6))

#     pygame.draw.rect(screen, RED, int(sw // 4 -130, 35, HEALTH_BAR_WIDTH, 20))
#     pygame.draw.rect(screen, GREEN, int(sw // 4 -130, 35, max(0, HEALTH_BAR_WIDTH * (f1.health/max(1,f1.max_health))),20))

#     pygame.draw.rect(screen, RED, int(3 * sw // 4 -130, 35, HEALTH_BAR_WIDTH, 20))
#     pygame.draw.rect(screen, GREEN, int(3* sw // 4 -130, 35, max(0, HEALTH_BAR_WIDTH * (f2.health / max(1,f2.max_health))), 20))


# def draw_message(msg, screen):
#     pygame.draw.rect(screen, WHITE, (100, 100, 100, 100))

#     surf = FONT_MED.render(msg, True, BLACK)
#     screen.blit(surf, (sw // 2 - surf.get_width() // 2, sh - MESSAGE_HEIGHT + (MESSAGE_HEIGHT - surf.get_height()) // 2))


# def draw_side(f:Fighter, left, screen):
#     panel_w = 150
#     x = 8 if left else sw - panel_w - 8
#     y = 130
...
#     stats = [
#         f"HP: {int(f.max_health)}",
#         f"Strength: {int(f.strength)}",
#         f"Defense: {int(f.defense)}",
#         f"Crit: {int(f.crit_chance)}",
#         f"Crit Bonus: {int(f.crit_bonus)}",
#         f"Attacks: {int(f.attacks)}",
#     ]

#     ty = y + 8

#     for line in stats:
#         screen.blit(FONT_SMALL.render(line, True, (30, 30, 30)), (x + 8, ty))
#         ty += 20
...
#     pos1 = (sw // 4 - surf1.get_width() // 2 + pos1_offset[0], sh // 2 - surf1.get_height() // 2 + pos1_offset[1]) 
#     pos2 = (3 * sw // 4 - surf2.get_width() // 2 + pos2_offset[0], sh // 2 - surf2.get_height() // 2 + pos2_offset[1])
...
#                 screen.blit(small, (x + 10, y + 10))

#             screen.blit(FONT_MED.render(f"{idx+1}. {f.name}", True, BLACK), (x + 100, y + 10))
#             screen.blit(FONT_SMALL.render(f"Class: {f.cls}", True, BLACK), (x + 100, y + 40))
#             screen.blit(FONT_SMALL.render(f"HP: {int(f.max_health)}", True, BLACK), (x + 100, y + 64))
...
def draw_ui(f1: Fighter, f2: Fighter, surface, interp_st_left=None, interp_st_right=None):
    ...
    if f1.health >= 99999999999:
        surface.blit(FONT_SMALL.render(f"{f1.name} (infinity HP)", True, BLACK),
                (WINDOW_WIDTH // 4 - 140, 6))
    else:
        surface.blit(FONT_SMALL.render(f"{f1.name} ({int(f1.health)} HP)", True, BLACK),
                (WINDOW_WIDTH // 4 - 140, 6))

    if f2.health >= 99999999999:
        surface.blit(FONT_SMALL.render(f"{f2.name} (infinity HP)", True, BLACK),
...
def draw_side(f: Fighter, left, surface):
    ...
    elif int(f.strength) == 999999999999:
        stats = [
            f"Class: {f.cls}",
...
def select_fighters_ui(fighters_list):
    ...
    while True:
        ...
        win.blit(
            FONT_MED.render("Select two people to have a friendly sparring match.", True, BLACK),
            (20, 10),
        )
